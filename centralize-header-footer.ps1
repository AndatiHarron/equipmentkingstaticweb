...
function Process-HTMLFile($filePath) {
    try {
        ...
        # Ensure we have the CSS link and header/footer loader
        $headSection = @"
<head>
    <meta charset="UTF-8">
    ...
    <script src="${cssPath -replace 'css/styles.css', 'js/header-footer-loader.js'}"></script>
</head>
"@
        ...
        if ($content -match '(?s)<head>.*?</head>') {
            ...
            $headSection = $headSection -replace '\{\{TITLE\}\}', $title
